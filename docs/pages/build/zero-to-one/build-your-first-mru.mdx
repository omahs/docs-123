...
- `schemas.ts` contains the [Action Schemas](/build/framework/action/action-schema) for your application.
...
=======
It is recommended that the `machine.ts` or the file containing the `StateMachine` instance, doesn't import anything from `@stackr/sdk` instead uses `@stackr/sdk/machine` for the `StateMachine`, `State`, `Transitions` and other utility classes.
