...
This file is created automatically during registration of the micro-rollup with the registry contract using [`stackr register`](/build/cli/register) command.
...
- `slotTime`: The time interval (in milliseconds) after which the rollup sends a block to Vulcan for verification and finalization.
...
- `accounts`: Contains the private key of the operator account. This private key is used for registration, deployment, and signing on incoming actions and proposed blocks.

---

6. `domain`: Contains the parameters for the [EIP-712](https://eips.ethereum.org/EIPS/eip-712) domain.
...
This is used for EIP-712 Typed Signature over the action payloads submitted by users.

:::note
This also means that the `domain` object here is incomplete and should not be used anywhere in your code. See [Accessing config values](/build/framework/config#accessing-config-values).
:::
